...
public class LibVLC extends VLCObject<ILibVLC.Event> implements ILibVLC {
    ...
    private HashMap<String,String> keyMaps=new HashMap<String,String>();
    ...
    //add httpvalues
    public void addHttpValue(String key,String value){
        if(!keyMaps.containsKey(key)){
            keyMaps.put(key,value);
        }
        String httpextend="";
        for(String mykey : keyMaps.keySet()){
            httpextend+=mykey+":"+keyMaps.get(mykey)+"\r\n";
        }
        nativeSetUserHttpExtend(httpextend);
    }
    ...
    private native void nativeSetUserHttpExtend(String extend);
